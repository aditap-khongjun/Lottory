...
namespace Lottory
{
    public partial class Summary_Customer_pay_Report : Form
    {
        private static Summary_Customer_pay_Report _instance;
        public static Summary_Customer_pay_Report Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new Summary_Customer_pay_Report();
                return _instance;
            }
        }
        private void Summary_Customer_pay_Report_FormClosed(object sender, FormClosedEventArgs e)
        {
            _instance = null;
        }
        public Summary_Customer_pay_Report()
        {
            InitializeComponent();
        }

        private void Summary_Customer_pay_Report_Load(object sender, EventArgs e)
        {
            ...
            this.BSCustomerTableInfo.DataSource = customerInfo;
            this.BSCustomerSummary.DataSource = summaryCustomer;
        ...
        private Summary_CustomerInfo getSummary(DataTable tableData)
        {
            ...
            {
                if (Convert.ToDouble(dr.ItemArray[5]) < 0)
                {
        ...
        private DataTable getCustomerList()
        {
            ...
            string sqlgetCustomerID = @"SELECT ci.CustomerID, MAX(ci.CustomerName) AS CustomerName
                                        FROM CustomerOrder c INNER JOIN CustomerInfo ci ON BINARY_CHECKSUM(c.CustomerID) = BINARY_CHECKSUM(ci.CustomerID)
        ...
        private double getPrice(string customerID)
        {
            ...
            string sqlgetPrice = string.Format(@"SELECT ISNULL(SUM(OwnPrice),0) AS Price
                                                 ...
                                                 WHERE BINARY_CHECKSUM(c.CustomerID) = BINARY_CHECKSUM('{0}')", customerID);
