...
namespace Lottory
{
    public partial class Summary_Customer_report_pay : Form
    {
        private static Summary_Customer_report_pay _instance;
        public static Summary_Customer_report_pay Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new Summary_Customer_report_pay();
                return _instance;
            }
        }
        private void Summary_Customer_report_pay_FormClosed(object sender, FormClosedEventArgs e)
        {
            _instance = null;
        }
        public Summary_Customer_report_pay()
        {
            InitializeComponent();
        }

        private void Summary_Customer_report_pay_Load(object sender, EventArgs e)
        {
            ...
            this.BSCustermerPayInfo.DataSource = customerInfo;
            this.BSSummary.DataSource = summaryCustomer;

            ReportParameterCollection reportParam = new ReportParameterCollection();
            reportParam.Add(new ReportParameter("OptionPay", "(ที่ต้องจ่าย)"));
            this.reportViewCustomerPay.LocalReport.SetParameters(reportParam);
            this.reportViewCustomerPay.RefreshReport();
        }
